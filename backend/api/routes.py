...
async def confirm_action(session_id: str, action_id: str, notes: str = ""):
    ...
    from services.confirmation_manager import get_confirmation_manager
    from services.confirmation_workflow import ConfirmationWorkflow

    mgr = get_confirmation_manager(session_id)
    result = await ConfirmationWorkflow(mgr).confirm(action_id, notes)
    if not result:
...
async def reject_action(session_id: str, action_id: str, notes: str = ""):
    ...
    mgr = get_confirmation_manager(session_id)
    result = await ConfirmationWorkflow(mgr).reject(action_id, notes)
    if not result:
        raise HTTPException(status_code=404, detail="Pending action not found")
    return {"status": result.status, "action": result.action.model_dump()}
...
async def correct_action(
    ...
    mgr = get_confirmation_manager(session_id)
    result = await ConfirmationWorkflow(mgr).correct(action_id, corrections or {}, notes)
    if not result:
        raise HTTPException(status_code=404, detail="Pending action not found")
    return {
        "status": result.status,
