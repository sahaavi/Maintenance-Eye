...
def _load_json() -> dict:
    ...
    if _json_cache:
        return _json_cache
    # __file__ is backend/api/routes.py
    # .parent is api/
    # .parent.parent is backend/
    # .parent.parent.parent is Maintenance-Eye/
    root_dir = Path(__file__).resolve().parent.parent.parent
    json_path = root_dir / "data" / "seed_data.json"
    ...
    if json_path.exists():
        ...
        logger.warning(f"seed_data.json not found at {json_path} for fallback")
    return _json_cache
...
async def search_assets(
    ...
    try:
        ...
        if results:
            return results
        raise Exception("empty response, try fallback")
    except Exception as e:
...
async def get_work_orders(asset_id: str = "", status: Optional[str] = None):
    """Get work orders, optionally filtered."""
    try:
        eam = get_eam_service()
        results = await eam.get_work_orders(asset_id=asset_id)
        if results:
            return results
        raise Exception("empty response, try fallback")
    except Exception as e:
...
async def get_eam_codes(code_type: str = "", department: str = "", asset_type: str = ""):
    ...
    try:
        eam = get_eam_service()
        results = await eam.get_eam_codes(
